...
#      puts "el atributo es: #{attribute}"
  ...
  def insert!
    ...
    puts "la tabla es: #{self.class.to_downcase_table}"
    insert_sql = "INSERT INTO #{self.class.to_downcase_table} (#{fields.join(',')}) VALUES (#{marks})"
    puts insert_sql
  ...
  def update!
    ...
    update_sql = "UPDATE #{self.class.to_downcase_table} SET #{update_clause} WHERE id = ?"
  ...
  private
    ...
    def self.to_downcase_table
      table = self.to_s.downcase + "s"
    end

    def self.all
      MiniActiveRecord::Model.execute("SELECT * FROM #{to_downcase_table}").map do |row|
        self.new(row)
      end
    end
  ...
  def self.where(query, *args)
    MiniActiveRecord::Model.execute("SELECT * FROM #{to_downcase_table} WHERE #{query}", *args).map do |row|
      Chef.new(row)
    end
  end
